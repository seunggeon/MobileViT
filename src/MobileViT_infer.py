...
from utils.images import read_dataset, read_dataset_infer
...
def mobilevit_infer(args_opt):
    ...
    for i, image in enumerate(dataset_infer.create_dict_iterator(output_numpy=True)):
        image = image["image"]
        image = Tensor(image[0:1], mindspore.float32)
        prob = model.predict(image)
        label = np.argmax(prob.asnumpy(), axis=1)
        # predict = dataset_analyse.index2label[int(label)]
        predict = dataset_analyse.get_imagenet_label(int(label))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_url", default="./datasets/", help="Location of data.")
    ...
    parser.add_argument("--infer_url", default='./image/', help="Location of inference data.")
