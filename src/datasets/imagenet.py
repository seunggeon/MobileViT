...
class ImageNet(Dataset):
    ...
    @property
    def get_imagenet_index_to_label(self):
        with open("./datasets/partial_imagenet_class_index.json") as f:
            class_idx = json.load(f)

        # int(label) → human-readable label
        index_to_label = {int(k): v[1] for k, v in class_idx.items()}

        return index_to_label
    
    def get_imagenet_label(self, label: int):
        index_to_label = self.get_imagenet_index_to_label
        return index_to_label.get(label, "Unknown label")
...
class ParseImageNet(ParseDataset):
    ...
    def parse_devkit(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            devkit_tar_path = "/home/pi/Desktop/MobileViT/data/ILSVRC2012_devkit_t12.tar.gz"  # devkit 경로 수정
