...
def read_dataset_infer(paths: str) -> Tuple[List[str], List[int]]:
    """
    Get the path list and index list of images.
    """
    img_list = list()
...
def read_dataset_infer(infer_url):
    image_list = [infer_url]
    image_label = [0]  # dummy label, inference doesn't use it
